...
const AgendaGPT = ({ currentUser, setSelectedClient }) => {
    ...
    const handleClientReport = async (clientName) => {
        ...
        try {
            const res = await api.exportToSheets(); // Currently exports all, but bridge can be updated
        ...
        <div className="flex flex-col h-full gap-8">
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 px-1">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-black text-brand-black tracking-tighter uppercase leading-none">Agenda Studio</h1>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mt-2">Contactos de LaGràfica</p>
                    </div>
                </div>
            ...
            {viewMode === 'chat' ? (
                <div className="flex-1 bg-white rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 p-8 flex flex-col items-center justify-center text-center max-w-2xl mx-auto space-y-6">
                        <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center text-brand-orange mb-4">
                            <Sparkles size={40} className="animate-pulse" />
                        </div>
                        <h2 className="text-2xl font-black text-gray-800 tracking-tight">Cerca qualsevol contacte dient coses com:<br />"Busca al client de Lleida en Verd"</h2>
                        <div className="w-full relative group">
                            <input
                                type="text"
                                placeholder="Escribe tu búsqueda aquí..."
                                className="w-full p-6 bg-gray-50 border border-gray-100 rounded-[2rem] text-lg font-bold outline-none focus:ring-2 focus:ring-brand-orange/20 pr-20 transition-all group-hover:bg-white group-hover:shadow-xl"
                            />
                            <button className="absolute right-4 top-4 bottom-4 bg-brand-orange text-white w-12 rounded-2xl flex items-center justify-center shadow-lg shadow-orange-500/20 hover:scale-105 transition-all">
                                <Send size={20} />
                            </button>
                        </div>
                    </div>
                ...
                <div className="flex flex-col gap-6 animate-in fade-in duration-300">
                    ...
                    <div className="flex flex-wrap justify-center gap-1">
                        <button
                            ...
                            className={`w-8 h-8 rounded-lg text-[10px] font-black transition-all ${!selectedLetter ? 'bg-brand-orange text-white shadow-lg shadow-orange-500/20' : 'bg-white text-gray-400 border border-gray-100'}`}
                        >
                            TOTS
                        </button>
                        {alphabet.map(l => (
                            <button
                                ...
                                className={`w-8 h-8 rounded-lg text-[10px] font-black transition-all ${selectedLetter === l ? 'bg-brand-orange text-white shadow-lg shadow-orange-500/20' : 'bg-white text-gray-400 border border-gray-100 hover:border-brand-orange/30'}`}
                            >
                                {l}
                            </button>
                        ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredContacts.map(contact => (
                            <div key={contact.id} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 hover:shadow-2xl hover:border-brand-orange/30 transition-all group relative overflow-hidden">
                                <div className={`absolute top-0 right-0 w-24 h-24 bg-gray-50 rounded-bl-[2.5rem] flex items-center justify-center transition-colors group-hover:bg-brand-orange/5`}>
                                    <User className="text-gray-200 group-hover:text-brand-orange/20" size={32} />
                                </div>

                                <span className={`inline-block mb-4 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${contact.tag === 'Cliente' ? 'bg-blue-50 text-blue-600 border border-blue-100' :
                                    contact.tag === 'Proveedor' ? 'bg-purple-50 text-purple-600 border border-purple-100' : 'bg-green-50 text-green-600 border border-green-100'
                                    }`}>
                                    {contact.tag}
                                </span>
                                ...
                                <div className="space-y-3 mb-8">
                                    <div className="flex items-center gap-3 text-xs font-bold text-gray-500">
                                        <div className="p-2 bg-gray-50 rounded-lg group-hover:bg-white shadow-sm transition-all"><Mail size={14} className="text-gray-300" /></div>
                                        <span className="truncate">{contact.email || 'No disponible'}</span>
                                    </div>
                                    <div className="flex items-center gap-3 text-xs font-bold text-gray-500">
                                        <div className="p-2 bg-gray-50 rounded-lg group-hover:bg-white shadow-sm transition-all"><Phone size={14} className="text-gray-300" /></div>
                                ...
                                {contact.tag === 'Cliente' && (
                                    <div className="flex flex-col gap-2 pt-6 mb-6 border-t border-gray-50">
                                        <button
                                            onClick={() => handleFilterProjects(contact.name)}
                                            className="flex items-center justify-between px-4 py-3 bg-blue-50/50 hover:bg-blue-50 text-blue-600 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all"
                                        >
                                            Ver Proyectos <LayoutDashboard size={14} />
                                        </button>
                                        <button
                                            onClick={() => handleClientReport(contact.name)}
                                            className="flex items-center justify-between px-4 py-3 bg-green-50/50 hover:bg-green-50 text-green-600 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all"
                                        >
                                            Informe Sheets <Table size={14} />
                                        </button>
                                    </div>
                                )}
                    ...
                    <form onSubmit={handleSave} className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-8 space-y-5">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="col-span-2">
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Empresa / Entidad</label>
                                    <input
                                        type="text"
                                        value={formData.company}
                                        onChange={e => setFormData({ ...formData, company: e.target.value })}
                                        className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-brand-orange/20 outline-none font-bold text-sm"
                                        placeholder="Ej: Generalitat"
                                    />
                                </div>
                            </div>
                            <div>
                                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Tipo de Contacto</label>
                                <div className="grid grid-cols-3 gap-2">
                                    {['Cliente', 'Proveedor', 'Referencia'].map(t => (
                                        <button
                                            key={t}
                                            type="button"
                                            onClick={() => setFormData({ ...formData, tag: t })}
                                            className={`py-3 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${formData.tag === t ? 'bg-brand-orange border-brand-orange text-white' : 'bg-white border-gray-100 text-gray-400'}`}
                                        >
                                            {t}
                                        </button>
                                    ))}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Correo</label>
                                    <input
                                        type="email"
                                        value={formData.email}
                                        onChange={e => setFormData({ ...formData, email: e.target.value })}
                                        className="w-full p-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-brand-orange/20 outline-none font-bold text-xs"
                            ...
                            <button
                                type="submit"
                                className="w-full bg-brand-black text-white font-black py-4 rounded-2xl hover:bg-brand-orange transition-all shadow-xl shadow-black/10 uppercase tracking-[0.2em] text-xs"
                            >
