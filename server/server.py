...
def read_db():
    if not os.path.exists(DB_FILE):
        ...
        initial_data = {
            "tasks": [
                {"id": 1, "title": "Rediseño Logo", "scope": "design", "status": "in_progress", "priority": "high", "client": "LaGràfica", "assignee": "neus", "boardId": "b_design", "columnId": "in_progress"},
                {"id": 2, "title": "Landing Page Evento", "scope": "web", "status": "todo", "priority": "medium", "client": "Ayto. BCN", "assignee": "montse", "boardId": "b_web", "columnId": "todo"},
                {"id": 3, "title": "Reels Enero", "scope": "social", "status": "review", "priority": "low", "client": "Café 365", "assignee": "alba", "boardId": "b_social", "columnId": "in_progress"}
            ],
            "members": [
            ...
            "boards": [
                {"id": "b_design", "title": "Disseny Gràfic", "columns": copy.deepcopy(default_columns)},
                {"id": "b_social", "title": "Xarxes Socials", "columns": copy.deepcopy(default_columns)},
                {"id": "b_web", "title": "Desenvolupament Web", "columns": copy.deepcopy(default_columns)},
                {"id": "b_ai", "title": "Projectes IA", "columns": copy.deepcopy(default_columns)},
                {"id": "b_lleida", "title": "Lleida en verd", "columns": copy.deepcopy(default_columns)},
                {"id": "b_animac", "title": "Animac", "columns": copy.deepcopy(default_columns)},
                {"id": "b_imo", "title": "Imo", "columns": copy.deepcopy(default_columns)},
                {"id": "b_diba", "title": "Diputació BCN", "columns": copy.deepcopy(default_columns)},
                {"id": "b_management", "title": "Gestió Interna", "columns": copy.deepcopy(default_columns)},
                {"id": "b_budget", "title": "Pressupostos", "columns": copy.deepcopy(default_columns)},
                {"id": "b_billing", "title": "Facturació", "columns": copy.deepcopy(default_columns)},
                {"id": "b_tenders", "title": "Licitacions", "columns": copy.deepcopy(default_columns)}
            ]
        }
        os.makedirs(os.path.dirname(DB_FILE), exist_ok=True)
        with open(DB_FILE, 'w') as f:
            json.dump(initial_data, f, indent=2)
        initial_data["cards"] = initial_data["tasks"]
        return initial_data
    ...
    with open(DB_FILE, 'r') as f:
        ...
        # Ensure Defaults
        if "processed_emails" not in data: data["processed_emails"] = []
        ...
        if "columns" not in data: data["columns"] = default_columns
        ...
        # Frontend Compat: Alias tasks -> cards
        data["cards"] = data.get("tasks", [])
        return data

def write_db(data):
    # Remove computed aliases before saving
    data_to_save = {k: v for k, v in data.items() if k != 'cards'}
    with open(DB_FILE, 'w') as f:
        ...
        try:
            ...
            for member in db['members']:
                ...
                if user and password:
                    ...
                    try:
                        script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fetch_mails.py')
                        proc = subprocess.run(['python3', script_path, user, password], capture_output=True, text=True)
                            ...
                            for em in emails:
                                ...
                                if do_auto_create:
                                    ...
                                    new_task = {
                                        ...
                                        "boardId": "b_management", # Default Board
                                        "columnId": "pending",
                                        "origin": "email",
...
class CustomHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed_path.path == '/api/boards':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.send_header('Access-Control-Allow-Origin', '*')
            self.end_headers()
            self.wfile.write(json.dumps(read_db().get('boards', [])).encode())
            return
        ...
        if parsed_path.path == '/api/users':
            ...
            self.send_header('Access-Control-Allow-Origin', '*')
            self.end_headers()
            self.wfile.write(json.dumps(read_db().get('members', [])).encode())
            return

        if parsed_path.path.startswith('/api/mailbox/') or parsed_path.path.startswith('/api/inbox/') or parsed_path.path.startswith('/api/emails/'):
            ...
            if user and password:
                try:
                    script_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fetch_mails.py')
                    proc = subprocess.run(['python3', script_path, user, password, folder], capture_output=True, text=True)
                    emails = json.loads(proc.stdout)
        ...
        if parsed_path.path == '/api/time_entries' or parsed_path.path.startswith('/api/time_entries?'):
            query = parse_qs(parsed_path.query)
            user_id = query.get('userId', [None])[0]
            
            db = read_db()
            entries = db.get('time_entries', [])
            if user_id:
    ...
    def do_PUT(self):
        ...
        # Alias cards -> tasks for PUT
        if parsed_path.path.startswith('/api/tasks/') or parsed_path.path.startswith('/api/cards/'):
        ...
        # Board Update (e.g. Columns)
        if parsed_path.path.startswith('/api/boards/'):
            board_id = parsed_path.path.split('/')[-1]
            content_length = int(self.headers['Content-Length'])
            payload = json.loads(self.rfile.read(content_length).decode())
            
            db = read_db()
            board = next((b for b in db.get('boards', []) if b['id'] == board_id), None)
            if board:
                ...
                self.send_header('Access-Control-Allow-Origin', '*')
                self.end_headers()
                self.wfile.write(json.dumps(board).encode())
            else:
                self.send_response(404)
                self.end_headers()
            return

        if parsed_path.path.startswith('/api/time_entries/'):
            entry_id = int(parsed_path.path.split('/')[-1])
            content_length = int(self.headers['Content-Length'])
            payload = json.loads(self.rfile.read(content_length).decode())
            
            db = read_db()
            if 'time_entries' not in db: db['time_entries'] = []
            
            entry = next((e for e in db['time_entries'] if e.get('id') == entry_id), None)
            if entry:
                for k, v in payload.items():
                    entry[k] = v
                write_db(db)
        ...
        if parsed_path.path.startswith('/api/users/'):
            ...
            db = read_db()
            user = next((u for u in db.get('members', []) if u['id'] == user_id), None)
            if user:
                for k, v in payload.items():
                    user[k] = v
                write_db(db)
    ...
    def do_POST(self):
        ...
        if parsed_path.path == '/api/upload':
            ...
            if 'file' in form:
                ...
                if fileitem.filename:
                    ...
                    self.send_header('Access-Control-Allow-Origin', '*')
                    self.end_headers()
                    self.wfile.write(json.dumps({"url": f"/uploads/{fn}", "filename": fn}).encode())
                    return
        ...
        if parsed_path.path == '/api/tasks' or parsed_path.path == '/api/cards':
            db = read_db()
            new_task = payload
        ...
        if parsed_path.path == '/api/convert-email':
            ...
            # Default Board Assigment based on Scope for better UX
            scope = new_task['scope']
            if scope == 'design': new_task['boardId'] = 'b_design'
            elif scope == 'web': new_task['boardId'] = 'b_web' 
            elif scope == 'social': new_task['boardId'] = 'b_social'
            else: new_task['boardId'] = 'b_management'
            new_task['columnId'] = 'pending'
        ...
        if parsed_path.path == '/api/columns':
            ...
            if 'id' not in new_col: new_col['id'] = str(int(time.time()))
        ...
        if parsed_path.path == '/api/time_entries':
            db = read_db()
            new_entry = payload
            if 'time_entries' not in db: db['time_entries'] = []
            
            # Simple ID generation
            if 'id' not in new_entry: new_entry['id'] = int(time.time() * 1000)
            db['time_entries'].append(new_entry)
            write_db(db)
        ...
        if parsed_path.path == '/api/archive-email':
            ...
            if user and password and uid:
                try:
                    script_path = os.path.join(os.getcwd(), 'server', 'fetch_mails.py')
                    proc = subprocess.run(['python3', script_path, user, password, '--archive', str(uid)], capture_output=True, text=True)
                    self.send_response(200)
                    self.send_header('Content-type', 'application/json')
                    self.send_header('Access-Control-Allow-Origin', '*')
                    self.end_headers()
